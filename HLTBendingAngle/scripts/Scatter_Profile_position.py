...
def denominator(dxy_min = 0, dxy_max = 5, eta_min = 1.6, eta_max = 1.7, minpt=0, maxpt = 500, stx=2, parity_case = 0):
...
#_____________________________________________________________________________________________________________________________________________________________________________________________________________________________
def numerator_Position(dxy_min = 0, dxy_max = 5, eta_min = 1.6, eta_max = 1.7, minpt=0, maxpt = 500, stationx=2, parity_case=0, minptcut=0):
...
def DeltaX23v12_Scatter(file, dir, xaxis, yaxis, x_bins, y_bins, etamin, etamax, minpt, maxpt, ctau, stationx, parity_case):
    ...
    if parity_case == 0:
        even_odd = "Odd_Even_Even"
        legend.AddEntry(0, "Odd in %s, Even in ME2 and ME3"%stationxx, "")
    if parity_case == 1:
        legend.AddEntry(0, "Odd Chambers in %s, ME2 and ME3"%stationxx, "")

    if parity_case == 2:
        legend.AddEntry(0, "Even Chambers in %s, ME2 and ME3"%stationxx, "")
    if parity_case == 3:
        legend.AddEntry(0, "Even in %s, Odd in ME2 and ME3"%stationxx, "")
        
    r1 = FitHistoFunction68(b1, xBins,xminBin,xmaxBin, yBins,yminBin,ymaxBin, 0)    
    r1.Fit("pol1", "EFM ", "same", 10, 120)
    r1.Fit("fxx")
...
def DeltaY_Scatter(file, dir, xaxis, yaxis, x_bins, y_bins, etamin, etamax, minpt, maxpt, ctau, stationx, parity_case):
    ...
    # Change X by Y or whatever you are using in labels!
    todrawb1 = "%s"%yaxis+":"+"%s>>b1"%xaxis
    b1 = TH2F("b1","b1",xBins,xminBin,xmaxBin,yBins,yminBin,ymaxBin)
    ...
    legend = TLegend(0.11,0.805,0.5,0.89)
    legend.SetFillColor(ROOT.kWhite)
    legend.SetMargin(0.01)
...
def DeltaY_Efficiency(file, dir, etamin, etamax, ctau, stationx, parity_case,  minptcut):
    ...
    eff_SimTrack_pt.SetLineColor(kBlue+3)
    eff_SimTrack_pt.SetMarkerStyle(22)
    eff_SimTrack_pt.SetMarkerColor(kBlue+3)
    ...
    if parity_case == 0:
        even_odd = "Odd_Even_Even"
        legend2.AddEntry(0, "Odd in %s, Even in ME2 and ME3"%stationxx, "")
    if parity_case == 1:
        legend2.AddEntry(0, "Odd Chambers in %s, ME2 and ME3"%stationxx, "")

    if parity_case == 2:
        legend2.AddEntry(0, "Even Chambers in %s, ME2 and ME3"%stationxx, "")
    if parity_case == 3:
        legend2.AddEntry(0, "Even in %s, Odd in ME2 and ME3"%stationxx, "")
...
#DeltaX23v12_Scatter("../ct%d.root"%ctau,tree,xaxis,yaxis,x_bins,y_bins, 1.6, 2.4, 10, 500 , ctau, 1, 0)
#DeltaX23v12_Scatter("../ct%d.root"%ctau,tree,xaxis,yaxis,x_bins,y_bins, 1.6, 2.4, 10, 500 , ctau, 1, 1)
#DeltaX23v12_Scatter("../ct%d.root"%ctau,tree,xaxis,yaxis,x_bins,y_bins, 1.6, 2.4, 10, 500 , ctau, 1, 2)
#DeltaX23v12_Scatter("../ct%d.root"%ctau,tree,xaxis,yaxis,x_bins,y_bins, 1.6, 2.4, 10, 500 , ctau, 1, 3)
...
for x in range (0, 4):
        ...
        xaxis = "pt_SimTrack"
        yaxis = "1/abs( (delta_y_gp_23) - %f*(delta_y_gp_12) )"%slopp
        #yaxis = "1/abs( (delta_y_23_from12) - %f*(delta_y_gp_12) )"%slopp

        #DeltaY_Scatter("../ct%d.root"%ctau,tree,xaxis,yaxis,x_bins,y_bins, 1.6, 1.8, 0, 500 , ctau, stxx, x)
        y_bins = "(280,0,14)" 
        #DeltaY_Scatter("../ct%d.root"%ctau,tree,xaxis,yaxis,x_bins,y_bins, 1.8, 2.0, 0, 500 , ctau, stxx, x)
        y_bins = "(500,0.0,25)" 
        #DeltaY_Scatter("../ct%d.root"%ctau,tree,xaxis,yaxis,x_bins,y_bins, 2.0, 2.2, 0, 500 , ctau, stxx, x)
        y_bins = "(1600,0,80)" 
        #DeltaY_Scatter("../ct%d.root"%ctau,tree,xaxis,yaxis,x_bins,y_bins, 2.2, 2.4, 0, 500 , ctau, stxx, x)



        DeltaY_Efficiency("../ct%d.root"%ctau,tree, 1.6, 1.8, ctau, stxx, x, 10)
        DeltaY_Efficiency("../ct%d.root"%ctau,tree, 1.8, 2.0, ctau, stxx, x, 10)
        DeltaY_Efficiency("../ct%d.root"%ctau,tree, 2.0, 2.2, ctau, stxx, x, 10)
        DeltaY_Efficiency("../ct%d.root"%ctau,tree, 2.2, 2.4, ctau, stxx, x, 10)
