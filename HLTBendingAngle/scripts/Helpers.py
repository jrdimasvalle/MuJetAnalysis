...
def Combined_pT_Simpler(etamin = 1.6, stx = 1, parity_case=0, minptcut=10):
    ...
    sigma = 1.0
    sigma_phi = 1.0
    return TCut("( (( 1/abs((delta_y_gp_23) - %f*(delta_y_gp_12) )  + %f )/%f ) -  2*(( 1/abs(csc_bending_angle_12) + %f )/%f ) ) <0   || "%(prop, intercept, slope, intercept_phi, slope_phi, minptcut ))
